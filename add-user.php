...
if (isset($_POST['register'])) {
    try {
        ...
        if (empty($nameExist)) {
            ...
            $insertStmt = $conn->prepare("INSERT INTO `tbl_user` (`user_id`, `first_name`, `last_name`, `contact_number`, `email`, `address` ,`username`, `password`, `verification_code`) VALUES (NULL, :first_name, :last_name, :contact_number, :email, :address, :username, :password, :verification_code)");
            ...
            <script>
                ...
                window.location.href = 'http://localhost/login/verification.php';
            </script>
            ";
...
if (isset($_POST['verify'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT `verification_code` FROM `tbl_user` WHERE `user_id` = :user_verification_id");
        $stmt->execute([
        ...
        if ($codeExist && $codeExist['verification_code'] == $verificationCode) {
            ...
            $conn->prepare("DELETE FROM `tbl_user` WHERE `user_id` = :user_verification_id")->execute([
                'user_verification_id' => $userVerificationID
            ]);
